    ...
    execute: async (client, interaction) => {
        if (interaction.type === InteractionType.ApplicationCommand) {
            try {
                ...
                if (command.userPermissions && !interaction.channel.permissionsFor(interaction.member).has(PermissionFlagsBits.resolve(command.userPermissions))) {
                    return interaction.reply({
                ...
                if (command.clientPermissions && !interaction.channel.permissionsFor(interaction.guild.members.me).has(PermissionFlagsBits.resolve(command.clientPermissions))) {
                    return interaction.reply({
            ...
            if (interaction.customId === 'automod-config') {
                const automodCommand = client.slashCommands.get('automod');
                if (automodCommand && automodCommand.handleModalSubmit) {
                    await automodCommand.handleModalSubmit(interaction);
                }
            }
